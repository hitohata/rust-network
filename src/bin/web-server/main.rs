use std::{collections::HashMap, io::{Write, BufReader}, io::Read, env, fs::File, process};
use mio::{tcp::{TcpListener, TcpStream}, Token, Poll, Ready, PollOpt, Events, Event};
use regex::Regex;

const SERVER: Token = Token(0);
const WEBROOT: &str = "/src/bin/web-server/webroot";
...
impl WebServer {
        ...
        loop {
            ...
            for event in &events {
                match event.token() {
                    ...
                    Token(conn_id) => {
                        self.http_handler(conn_id, event, &poll, &mut response).unwrap_or_else(|e| log::error!("{}", e));
                    }
                }
            }
        }
    ...
    fn register_connection(
        ...
        if self.connections.insert(self.next_connection_id, stream).is_some() {
            log::error!("Connection ID is already exist.");
        };
    ...
    fn http_handler(
        ...
        if event.readiness().is_readable() {
            log::debug!("readable conn_id: {}", conn_id);
            ...
            log::debug!("writable conn_id: {}", conn_id);
...
fn make_response(buffer: &[u8]) -> Result<Vec<u8>, failure::Error> {
        ...
        let captures = match http_pattern.captures(std::str::from_utf8(buffer)?) {
            Some(cap) => cap,
            None => {
